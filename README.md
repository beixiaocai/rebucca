...
- 布控报警：多边形区域、7 种后处理规则（入侵/越线/计数/方向/密度/滞留/离岗）
...
### v1.005
- 2026/09/24
- **新增「离岗检测」后处理规则（第 7 种业务规则）**
  - 与密度报警相反的反向区域规则：密度是「区域内目标数 ≥ 阈值」报警，离岗是「区域内连续 N 秒无目标」报警。
  - 在业务算法页选择「离岗检测」，在布控区域配置离岗阈值（秒）即可使用；区域内连续无匹配目标达到阈值触发报警，目标回岗后自动重新武装、可再次触发。
  - 检测目标类别沿用业务算法的「检测目标」配置（如 person），只有匹配类别的目标才算「在岗」。
  - 适用场景：机房值守、收银台离岗、值班岗在岗监测、工地值班、客服工位监督等。
  - 实现：`models.py` 新增 `POST_ABSENCE` 与 `ZoneModel.absence_threshold`；`biz_rules.py` 新增 ABSENCE 规则匹配与离岗报警文案；`pipeline.py` 新增区域级离岗判定（在场计时/回岗重新武装）；`schema_upgrade.py` 老库自动补列，无需手动迁移。
- **补齐密度报警阈值配置入口（重要修复）**
  - 此前密度报警（DENSITY）的 `density_threshold` 只有后端字段，布控区域编辑界面没有配置入口，导致密度报警阈值恒为 0、永远无法触发。
  - 现已在布控区域弹框中增加「密度阈值」「离岗阈值」两个输入项，新建/编辑均可配置并随布控保存。
- **报警管理支持密度/离岗类型筛选**
  - 报警列表新增「密度报警」「离岗检测告警」筛选选项与类型显示。
- **区域级报警快照优化**
  - 密度/离岗等区域级报警本身没有具体目标框，快照不再在左上角绘制 0×0 空检测框，仅保留区域多边形与事件角标。
- **国际化**
  - 新增离岗/密度相关翻译键并补齐至全部 7 种语言；补齐简体中文缺失的 `zone_state_enabled` / `zone_state_disabled`。
